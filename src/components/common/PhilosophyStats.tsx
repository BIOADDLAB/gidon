...
export default function PhilosophyStats() {
    ...
    return (
        <RevealStagger
            className="mx-auto mt-10 grid max-w-[760px] grid-cols-1 gap-4 px-4 md:mt-12 md:grid-cols-2"
            y={18}
            stagger={0.12}
        >
            <div className="rounded-[28px] border border-green-600/20 bg-white/80 px-6 py-7 text-center shadow-sm backdrop-blur-sm">
                <p className="text-base font-semibold text-green-600 md:text-lg">합리적인 임플란트 치료 환자분</p>
                <strong className="mt-3 block font-accent text-[48px] font-normal leading-none text-green-900 md:text-[62px]">
                    <CountUpNumber value={stats.patients} />
                </strong>
                <span className="mt-2 block text-base font-bold text-ink-500">명</span>
            </div>
            <div className="rounded-[28px] border border-green-600/20 bg-white/80 px-6 py-7 text-center shadow-sm backdrop-blur-sm">
                <p className="text-base font-semibold text-green-600 md:text-lg">임플란트 식립 개수</p>
                <strong className="mt-3 block font-accent text-[48px] font-normal leading-none text-green-900 md:text-[62px]">
                    <CountUpNumber value={stats.implants} />
                </strong>
                <span className="mt-2 block text-base font-bold text-ink-500">개</span>
            </div>
        </RevealStagger>
    );
}
